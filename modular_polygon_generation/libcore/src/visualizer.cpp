...
void Visualizer::DrainQueuesOnGuiThread_() {
    ...
    while (!nodes_local.empty()) {
        ...
        if (vis_nodes) {
            ...
            logger::debug << "Adding Vis sphere " << sphere_name << std::endl;
        ...
        if (vis_edges) {
            logger::debug << "Adding Vis lines for " << sphere_name << std::endl;
        ...
        if (vis_black_vertices) {
             logger::debug << "Adding Vis black vertices for " << sphere_name << std::endl;
            // Black vertices (blue)
            for (const auto& v : node->black_vertices) {
                ...
                logger::debug << "Adding Vis black vertex at " << v->coord.transpose() << std::endl;
                // check if name already exists
                if (scene_->HasGeometry(name.c_str())) {
                    logger::error << "Geometry with name " << name << " already exists. Skipping." << std::endl;
                    continue;
                }
        ...
        if (vis_white_vertices) {
            logger::debug << "Adding Vis white vertices for " << sphere_name << std::endl;
        ...
        if (vis_facets) {
            logger::debug << "Adding Vis facets for " << sphere_name << std::endl;
            // add facets as mesh
        ...
        if (vis_frontiers) {
            logger::debug << "Adding Vis frontiers for " << sphere_name << std::endl;
            for (const auto& f : node->frontiers) {
                ...
                for (const auto& facet : f->facets) {
                    ...
                    if (facet->vertices[0]->coord == facet->vertices[1]->coord ||
                        ...
                        logger::error << "Found duplicate vertex coordinates in facet for node " << node->debug_id << std::endl;
                        continue;
                    }
    ...
    while (!debug_local.empty()) {
        ...
        logger::debug << "Adding Debug sphere " << name << std::endl;
        scene_->AddGeometry(name.c_str(), s.get(), mat_debug_, false);
    }
}
...
void Visualizer::setSettings(bool nodes, bool edges, bool black_vertices,
                             bool white_vertices, bool facets, bool frontiers,
                             bool debug_nodes) {
    vis_nodes = nodes;
    vis_edges = edges;
    vis_black_vertices = black_vertices;
    vis_white_vertices = white_vertices;
    vis_facets = facets;
    vis_frontiers = frontiers;
    vis_debug_nodes = debug_nodes;
}
